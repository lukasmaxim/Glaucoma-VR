...
namespace Varjo
{
    public class VarjoPluginMR
    {
        ...
        private static bool MRNotAvailableError = false;
        ...
        private static bool IsMRAvailable(bool displayError)
        {
            ...
            if (!MRIsAvailable())
            {
                if (displayError && !MRNotAvailableError)
                {
                    Debug.LogError("Mixed reality hardware not available.");
                    MRNotAvailableError = true;
                }
                return false;
            }
            MRNotAvailableError = false;
            return true;
        }

        /// <summary>
        /// Checks if mixed reality hardware is available.
        /// </summary>
        /// <returns>True, if Varjo session is ready and mixed reality hardware is available.</returns>
        public static bool IsMRAvailable()
        {
            return IsMRAvailable(false);
        }
        ...
        {
            if (!IsMRAvailable(true)) return 0;
