Shader "Debug/MoveTexture"{
    ...
    float4 FragDefault(VaryingsDefault i) : SV_Target
    {
        ...
        if(screen == -1)
        {
            maskValue = SAMPLE_TEXTURE2D(_MaskTexContext, sampler_MaskTexContext, i.texcoord + offset);
        }
        else
        {
            maskValue = SAMPLE_TEXTURE2D(_MaskTexFocus, sampler_MaskTexFocus, float2(i.texcoord.x * aspect * 1/scaleFactor + offset.x, i.texcoord.y * 1/scaleFactor + offset.y));
        }
