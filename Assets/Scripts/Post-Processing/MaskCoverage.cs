...
public sealed class MaskCoverage : PostProcessEffectSettings
{
    ...
    public ColorParameter overlayColor = new ColorParameter { value = new Color(1.0f, 1.0f, 0.0f, 1.0f) };
    [Range(0.0f, 1.0f)]
    public FloatParameter alphaCutoff = new FloatParameter { value = 0.2f };
...
public sealed class MaskCoverageRenderer : PostProcessEffectRenderer<MaskCoverage>
{
    ...
    void ApplyToCamera(PostProcessRenderContext context, PropertySheet sheet, int eye, bool invalid, Vector3 gazeOrigin, Vector3 gazeDirection, float aspect, float scaleFactor, Vector2 offset, int screen)
    {
        ...
        if (!invalid)
        {
            ...
            sheet.properties.SetFloat("_AlphaCutoff", settings.alphaCutoff);
        }
        else
        {
            sheet.properties.SetInt("eye", eye);
    ...
    void InitialSetup(PostProcessRenderContext context)
    {
        if (!set)
        {
            ...
            this.sheet = context.propertySheets.Get(Shader.Find("Debug/MaskCoverage"));
            ...
            this.sheet.properties.SetColor("_OverlayColor", settings.overlayColor);
            set = true;
        }
    }
